...
export default function NotificationSettingsTab({ data, onChange }: NotificationSettingsTabProps) {
  ...
  const handleSendTestMessage = async () => {
    ...
    try {
      ...
      if (!sent) {
        try {
          const directRes = await fetch("https://slick-pumas-jump.loca.lt/send", {
            method: "POST",
            headers: {
  ...
  const fetchWaBotInfo = async () => {
    ...
    try {
      ...
      if (finalStatus === "DISCONNECTED") {
        try {
          const tunnelRes = await fetch("https://slick-pumas-jump.loca.lt/status", {
            cache: "no-store",
          ...
          if (tunnelRes?.status && tunnelRes.status !== "DISCONNECTED") {
            ...
            setWaBotUrl("https://slick-pumas-jump.loca.lt");
          }
        } catch {}
      }
      ...
      if (finalStatus === "QR_READY" && !finalQr) {
        try {
          const tunnelQrRes = await fetch("https://slick-pumas-jump.loca.lt/qr", {
            cache: "no-store",
            headers: { "Bypass-Tunnel-Reminder": "true" }
          }).then((r) => r.json()).catch(() => null);
          if (tunnelQrRes?.qr) {
